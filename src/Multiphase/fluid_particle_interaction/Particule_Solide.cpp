...
Entree& Particule_Solide::readOn(Entree& is)
{
  ...
  if (monodisperse_==1)
    {
      set_diametre(2*rayon_);
      set_volume_compo(4 * M_PI * pow(rayon_, 3) / 3);
      const double rho_solide = masse_volumique().valeurs()(0, 0);
      set_masse_compo((4 * M_PI * pow(rayon_, 3) / 3)*rho_solide);
    }
  return is;
}
...
void Particule_Solide::set_param(Param& param)
{
  ...
  param.ajouter("rayon", &rayon_); // XD_ADD_P radius of a spherical particle;
  param.ajouter("monodisperse", &monodisperse_);
}

const double& Particule_Solide::e_dry() const { return e_dry_; }
void Particule_Solide::set_rayon(const double rayon) const { rayon_=rayon; }
...
const double& Particule_Solide::rayon() const { return rayon_; }
